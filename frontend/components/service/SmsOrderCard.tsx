...
function formatPhone(
  ...
  const localNumber =
    dialcodewithoutPlus && normalized.startsWith(dialcodewithoutPlus)
      ? normalized.slice(dialcodewithoutPlus.length)
      : normalized.startsWith("+")
        ? normalized.slice(1)
        : normalized;
  if (!dialCode || !countryName) return { local:phoneNumber};
  return { country: `${countryName} (${dialCode})`, local: localNumber };
}

export default function SmsOrderCard({
  order,
  onChange,
  onRemove,
}: Props) {
  ...
  const displayPhone = useMemo(
    () => formatPhone(order.phoneNumber, order.countryCode, order.countryName),
    [order.phoneNumber, order.countryCode, order.countryName],
  );
  ...
  const otpText = useMemo(
    () => order.otpCode || "در انتظار دریافت پیامک...",
    [order.otpCode],
  );
  ...
  async function resend() {
    ...
    setAction("resend");
    try {
      onChange?.(await resendOrder(providerOrderId));
  ...
  return (
    <article
      className={`min-w-[290px] rounded-[24px] border border-white/10 bg-white/[.045] p-4 shadow-xl shadow-black/10 w-full`}
      dir="rtl"
    >
      ...
      <div className="mt-3 flex flex-col  gap-2 rounded-2xl bg-black/20 px-3 py-2.5">
        <span className="text-cyan-300/80">کشور</span>
        <span>{displayPhone.country}</span>
      </div>
      <div className="mt-3 flex items-center gap-2 rounded-2xl bg-black/20 px-3 py-2.5">
        <span
          className="min-w-0 flex-1 truncate font-mono text-base font-black tracking-wide text-white"
          dir="ltr"
        >
          {displayPhone.local}
        </span>
        <button
          ...
          className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-white/[.06] text-white/65 disabled:opacity-30"
      ...
      <div className="mt-3 rounded-2xl border border-white/8 bg-white/[.025] px-3 py-2.5">
        <div className="flex items-center justify-between gap-2 text-[10px] text-white/35">
          <span>آخرین کد دریافت‌شده</span>
          <span>{order.smsRevision ? `پیامک ${order.smsRevision}` : ""}</span>
        </div>
        <p className="mt-1 font-mono text-sm font-black tracking-widest text-cyan-200">
          {otpText}
        </p>
        {order.otpMessage ? (
          <p className="mt-1 line-clamp-2 text-[10px] leading-5 text-white/40">
            {order.otpMessage}
          </p>
        ) : null}
      </div>
      ...
      <div className="mt-3 grid grid-cols-2 gap-2">
        <button
          type="button"
          onClick={() => void resend()}
          disabled={!order.canResend || Boolean(action)}
          className="flex h-9 items-center justify-center gap-1.5 rounded-xl bg-white/[.06] text-[10px] font-bold text-white/70 disabled:opacity-30"
        >
          <RefreshCw
            size={13}
            className={action === "resend" ? "animate-spin" : ""}
          />{" "}
          ارسال مجدد
        </button>
        <button
          type="button"
          onClick={() => void cancel()}
          disabled={!order.canCancel || Boolean(action) || !order.phoneNumber}
          className="flex h-9 items-center justify-center gap-1.5 rounded-xl bg-red-400/10 text-[10px] font-bold text-red-200 disabled:opacity-30"
        >
          {action === "cancel" ? (
      ...
      {copied ? (
        <p className="mt-2 text-center text-[10px] text-cyan-200">
          شماره کپی شد
        </p>
      ) : null}
    </article>
