...
export default function AppleServicePurchase({
    ...
    return (
      <section className="mt-5 rounded-[28px] border border-white/10 bg-white/[.035] p-5 sm:p-7">
        ...
        <button
          ...
          className="mt-7 w-full rounded-2xl bg-cyan-300 px-5 py-3.5 text-sm font-black text-slate-950 transition hover:bg-cyan-200"
        >
          شروع
        </button>
      </section>
  ...
  return (
    <section className="mt-5 rounded-[28px] border border-white/10 bg-white/[.035] p-4 sm:p-6">
      <div className="mb-5 flex items-center justify-between">
        <div>
        ...
        <span className="rounded-full bg-white/[.06] px-3 py-1 text-[11px] text-white/45">
          {product.title}
        </span>
      </div>
      ...
      <div className="mt-7 flex items-center justify-between gap-3">
        <button
          ...
          className="inline-flex items-center gap-1.5 rounded-xl border border-white/10 px-3 py-2 text-xs text-white/55 disabled:opacity-25"
        >
        ...
        <div className="flex items-center gap-1.5">
          {steps.map((item, index) => (
            <span
              key={item.id}
              className={`h-1.5 w-1.5 rounded-full ${index === step ? "bg-cyan-300 scale-125" : "bg-white/20"}`}
            />
          ))}
        </div>
        <button
      ...
      {step === steps.length - 1 && (
        <div className="mt-5 flex items-center justify-center gap-2 rounded-2xl bg-emerald-400/10 py-3 text-xs text-emerald-300">
          <CheckCircle2 size={16} /> راهنما به پایان رسید
        </div>
      )}
    </section>
  );
