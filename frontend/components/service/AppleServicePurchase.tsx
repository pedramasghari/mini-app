...
import { ArrowLeft, ArrowRight, Loader2, Phone, PlayCircle } from "lucide-react";
...
const TERMINAL_STATUSES = new Set(["CANCELED", "CANCELLED", "EXPIRED", "COMPLETED", "SUCCESS"]);
...
export default function AppleServicePurchase({ service, product, guide }: Props) {
  ...
  const [smsOrder, setSmsOrder] = useState<SmsOrderCardData | null>(null);
  ...
  const clearSmsOrder = useCallback(() => {
    setSmsOrder(null);
    setTimer(0);
  ...
  useEffect(() => {
    try {
      ...
      if (saved) {
        ...
        if (typeof saved.step === "number") setStep(Math.max(0, Math.min(Math.max(steps.length - 1, 0), saved.step)));
      }
    } catch {
  ...
  const syncSmsOrder = useCallback(async (id: string, silent = true) => {
    try {
      ...
      // The backend is the source of truth. Once an order becomes terminal,
      // immediately remove the number card so a canceled/expired number can
      // never remain visible as an active number.
      ...
      if (next.status !== lastStatus.current && next.status === "OTP_RECEIVED" && (next.smsRevision ?? 0) > notifiedRevision.current) {
  ...
  useEffect(() => {
    if (!smsOrder?.expiresAt || isTerminal(smsOrder.status)) return;
    const tick = window.setInterval(() => {
      const seconds = remainingSeconds(smsOrder.expiresAt);
      setTimer(seconds);
      if (seconds === 0 && ACTIVE_STATUSES.has(smsOrder.status)) void syncSmsOrder(smsOrder.id);
    }, 1000);
    return () => window.clearInterval(tick);
  }, [smsOrder?.id, smsOrder?.expiresAt, smsOrder?.status, syncSmsOrder]);
  ...
  async function requestNumber() {
    ...
    try {
      ...
      if (isTerminal(next.status)) {
        clearSmsOrder();
        toast.error("شماره قابل استفاده نیست. لطفاً دوباره تلاش کنید.");
        return;
      }
      setSmsOrder(next);
      setTimer(remainingSeconds(next.expiresAt));
  ...
  const guideSection = !guide || !steps.length || product.requiresGuide === false ? (
    <section className="flex h-[560px] flex-col rounded-[28px] border border-white/10 bg-white/[.035] p-5 sm:p-7">
      <div className="flex items-center gap-3">
        <div className="grid h-11 w-11 place-items-center rounded-2xl bg-cyan-300/10 text-cyan-200"><PlayCircle size={22} /></div>
        <div><span className="text-xs text-cyan-300">سرویس</span><h2 className="mt-1 text-lg font-black">{product.title}</h2></div>
      </div>
    ...
    <section className="h-[700px] overflow-hidden rounded-[28px] border border-white/10 bg-white/[.035] p-4 sm:p-6">
      {!started ? (
        ...
        <div className="flex h-full flex-col">
          ...
          <div className="mt-4 flex min-h-0 flex-1 flex-col overflow-hidden"><AnimatePresence mode="wait" initial={false}>{current && <motion.div key={current.id} initial={{ opacity: 0, x: 14 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -14 }} transition={{ duration: 0.16 }} className="flex h-full min-h-0 flex-col"><div className="h-[285px] shrink-0 overflow-hidden rounded-2xl border border-white/10 bg-black/20">{current.mediaUrl ? current.mediaType === "video" ? <video className="h-full w-full object-contain" src={current.mediaUrl} controls playsInline preload="metadata" /> : <img className="h-full w-full object-contain" src={current.mediaUrl} alt={current.title} loading="eager" decoding="async" /> : <div className="grid h-full place-items-center text-xs text-white/25">این مرحله مدیا ندارد</div>}</div><div className="mt-4 min-h-0 flex-1 overflow-y-auto px-1 pb-2"><h3 className="text-lg font-black">{current.title}</h3><p className="mt-3 whitespace-pre-line text-sm leading-8 text-white/60">{current.content}</p>{current.requiresInput && <div className="mt-5"><label className="mb-2 block text-xs font-bold text-white/60">{current.inputLabel || "اطلاعات مورد نیاز"}</label><input className="w-full rounded-2xl border border-white/10 bg-white/[.045] px-4 py-3 text-sm outline-none focus:border-cyan-300/50" placeholder={current.inputLabel || ""} /></div>}</div></motion.div>}</AnimatePresence></div>
...
function statusText(status: string) {
  ...
  if (status === "CANCELED" || status === "CANCELLED") return "سفارش لغو شد.";
  if (status === "EXPIRED") return "سفارش منقضی شد.";
  if (status === "COMPLETED" || status === "SUCCESS") return "سفارش تکمیل شد.";
  return `وضعیت سفارش: ${statusLabel(status)}`;
}
