...
type StatusResponse = {
  id: string; ticketId: string; trackId: string | null; status: PaymentStatus; amount: string; currency: string; expiresAt: string | null;
  gateway?: { result: string | null; status: number | null; message: string | null; refNumber: string | null; cardNumber: string | null; paidAt: string | null };
};
...
function ZibalStatusContent() {
  ...
  const checkStatus = useCallback(async () => {
    if (!ticketId || checking) return;
    setChecking(true);
    try {
      const data = await api<StatusResponse>(`zibal/payments/${encodeURIComponent(ticketId)}/verify`, { method: 'POST', headers: { 'Content-Type': 'application/json' } });
      setPayment(data);
      setError('');
  ...
  useEffect(() => {
    if (!payment?.expiresAt || payment.status !== 'PENDING') { setRemainingSeconds(null); return; }
    const update = () => setRemainingSeconds(Math.max(0, Math.floor((new Date(payment.expiresAt!).getTime() - Date.now()) / 1000)));
    update();
  ...
  if (!ticketId) return (
    <main className="mx-auto w-full max-w-2xl px-3 pb-24 pt-8 sm:px-5"><section className="rounded-[32px] border border-red-400/20 bg-[#111827] p-7 text-center text-white shadow-2xl"><XCircle className="mx-auto h-16 w-16 text-red-300" /><h1 className="mt-5 text-2xl font-black">شناسه پرداخت وجود ندارد</h1><p className="mt-3 text-sm leading-7 text-white/50">برای مشاهده وضعیت، این صفحه باید با Ticket ID باز شود.</p><button onClick={back} className="mt-7 w-full rounded-2xl bg-white px-4 py-4 text-sm font-black text-black">بازگشت به پنل</button></section></main>
  );
  ...
  return (
    <main className="mx-auto w-full max-w-2xl px-3 pb-24 pt-8 sm:px-5"><section className="overflow-hidden rounded-[32px] border border-white/10 bg-[#111827] text-white shadow-2xl">
      <div className="border-b border-white/10 p-5"><button onClick={back} className="flex items-center gap-2 rounded-xl bg-white/5 px-3 py-2 text-sm font-bold text-white/70"><ArrowRight size={17} /> بازگشت</button></div>
      <div className="p-7 text-center">
        {isSuccess ? <CheckCircle2 className="mx-auto h-20 w-20 text-emerald-300" /> : isTerminalFailure ? <XCircle className="mx-auto h-20 w-20 text-red-300" /> : <div className="mx-auto grid h-20 w-20 place-items-center rounded-full bg-amber-400/10"><Clock3 className="h-10 w-10 animate-pulse text-amber-300" /></div>}
        <p className="mt-6 text-xs font-bold text-cyan-300/70">وضعیت پرداخت زیبال</p><h1 className="mt-2 text-2xl font-black">{title}</h1><p className="mx-auto mt-3 max-w-lg text-sm leading-7 text-white/50">{description}</p>
        <div className="mt-7 rounded-3xl border border-white/10 bg-white/[.03] p-5 text-right">
          ...
          {payment && <>
            ...
            <div className="mt-3 flex items-center justify-between text-sm"><span className="text-white/40">وضعیت</span><strong className={isSuccess ? 'text-emerald-300' : isTerminalFailure ? 'text-red-300' : 'text-amber-300'}>{statusLabel}</strong></div>
            ...
            {payment.gateway?.status !== null && payment.gateway?.status !== undefined && <div className="mt-3 flex items-center justify-between gap-4 text-sm"><span className="text-white/40">وضعیت زیبال</span><span dir="ltr" className="font-mono text-xs text-white/55">{payment.gateway.status}</span></div>}
...
export default function ZibalStatusPage() { return <Suspense fallback={<main className="mx-auto max-w-2xl px-3 pt-12 text-center text-white/60">در حال بارگذاری وضعیت پرداخت…</main>}><ZibalStatusContent /></Suspense>; }
