...
type Status = 'PENDING' | 'COMPLETED' | 'CANCELLED' | 'APPROVED' | 'REJECTED';
...
type Filter = 'ALL' | 'PENDING' | 'WITHDRAWAL' | 'DEPOSIT' | 'COMPLETED' | 'CANCELLED' | 'REJECTED';
...
function statusClass(item: RequestItem) {
  ...
  if (item.status === 'APPROVED' || item.status === 'COMPLETED') return 'bg-emerald-400/10 text-emerald-200';
...
export default function WalletRequestsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (['PENDING', 'COMPLETED', 'CANCELLED', 'REJECTED'].includes(filter)) params.set('status', filter);
    ...
    <main dir="rtl" className="mx-auto w-full max-w-5xl space-y-5 p-4 pb-24">
      <header className="rounded-3xl border border-white/10 bg-white/[0.04] p-5 shadow-xl">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div><p className="text-sm text-white/45">کیف پول</p><h1 className="mt-1 text-2xl font-bold">درخواست‌های کیف پول</h1><p className="mt-2 text-sm text-white/50">درخواست‌های شارژ و برداشت در یک لیست؛ موارد در حال انجام همیشه بالاتر هستند.</p></div>
      ...
      <section className="space-y-3">
        ...
        {!loading && data?.items.map((item) => (
          <article key={`${item.kind}-${item.id}`} className="rounded-3xl border border-white/10 bg-white/[0.035] p-4 transition hover:border-white/20">
            ...
            {item.kind === 'DEPOSIT' && item.receiptPath && <div className="mt-4 rounded-2xl bg-black/15 p-3 text-sm text-white/55">رسید پرداخت ثبت شده است.</div>}
