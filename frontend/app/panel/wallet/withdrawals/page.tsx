...
const statusText: Record<Withdrawal['status'], string> = {
...
export default function WithdrawalsPage() {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      await api<Withdrawal>('wallet/withdrawals', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ amount, cardNumber, cardHolderName }),
      });
      setAmount(''); setCardNumber(''); setCardHolderName(''); setPage(1);
  ...
  async function cancel(id: string) {
    ...
    try {
      await api(`wallet/withdrawals/${id}/cancel`, { method: 'POST' });
      setSuccess('درخواست لغو شد و مبلغ به کیف پول بازگشت.');
      await Promise.all([load(), refresh()]);
    ...
    <main className="mx-auto w-full max-w-3xl space-y-5 p-4 pb-24 text-right" dir="rtl">
      <header className="rounded-3xl border border-white/10 bg-white/[0.04] p-5 shadow-xl">
        ...
        <div className="mt-2 flex items-end justify-between gap-4">
          <div><h1 className="text-2xl font-bold">برداشت وجه</h1><p className="mt-1 text-sm text-white/55">درخواست برداشت را ثبت کنید؛ مبلغ تا زمان واریز نزد سیستم رزرو می‌شود.</p></div>
          <div className="rounded-2xl bg-white/[0.06] px-4 py-3 text-left"><div className="text-xs text-white/45">موجودی</div><div className="mt-1 font-bold">{fa(me?.wallet?.balance ?? 0)} {me?.wallet?.currency ?? 'IRT'}</div></div>
        </div>
      </header>

      <section className="rounded-3xl border border-white/10 bg-white/[0.035] p-5">
        <h2 className="text-lg font-semibold">درخواست برداشت جدید</h2>
        <form onSubmit={submit} className="mt-5 space-y-4">
          ...
          {success && <div className="rounded-2xl border border-emerald-400/20 bg-emerald-400/10 p-3 text-sm text-emerald-200">{success}</div>}
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between"><h2 className="text-lg font-semibold">درخواست‌های من</h2><span className="text-xs text-white/40">{data?.total ?? 0} درخواست</span></div>
        {data?.items.map((item) => (
