...
export class WithdrawalsService {
  ...
  async create(userId: string, input: { cardNumber: string; cardHolderName: string; amount: string }) {
    ...
    const result = await this.dataSource.transaction(async (manager) => {
      ...
      const wallet = await walletRepo.createQueryBuilder('w').where('w.userId = :userId', { userId }).andWhere('w.currency = :currency', { currency: CURRENCY }).setLock('pessimistic_write').getOne();
      if (!wallet) throw new NotFoundException('کیف پول پیدا نشد.');
      const balanceBefore = normalizeDecimal(wallet.balance);
      const updated = await manager.query(`UPDATE wallets SET balance = balance - CAST($1 AS numeric), "updatedAt" = NOW() WHERE id = $2 AND balance >= CAST($1 AS numeric)`, [amount, wallet.id]);
      ...
      const saved = await manager.getRepository(WithdrawalRequest).save(manager.getRepository(WithdrawalRequest).create({ userId, cardNumber, cardHolderName, amount, currency: CURRENCY, status: 'PENDING' }));
      await manager.getRepository(WalletTransaction).save(manager.getRepository(WalletTransaction).create({ userId, walletId: wallet.id, type: WITHDRAW, amount: `-${amount}`, balanceBefore, balanceAfter, currency: CURRENCY, referenceType: REFERENCE, referenceId: saved.id, description: 'درخواست برداشت وجه' }));
      return saved;
    });
    try { await this.notifications.create(userId, { type: 'WITHDRAWAL_PENDING', title: 'درخواست برداشت ثبت شد', message: `درخواست برداشت ${result.amount} ${result.currency} ثبت شد و در انتظار واریز است.`, data: { withdrawalId: result.id, amount: result.amount, status: result.status } }); } catch {}
    return result;
  }

  async listMine(userId: string, page = 1, limit = 10) {
    page = Math.max(1, page); limit = Math.min(50, Math.max(1, limit));
    const [items, total] = await this.withdrawals.findAndCount({ where: { userId }, order: { createdAt: 'DESC' }, skip: (page - 1) * limit, take: limit });
  ...
  async listWalletRequests(userId: string, page = 1, limit = 10, type = 'ALL', status?: string) {
    ...
    if (!['ALL', 'WITHDRAWAL', 'DEPOSIT'].includes(normalizedType)) throw new BadRequestException('نوع درخواست نامعتبر است.');
  ...
  async cancel(userId: string, id: string) {
    const result = await this.dataSource.transaction(async (manager) => {
      const walletRepo = manager.getRepository(Wallet);
      const request = await manager.getRepository(WithdrawalRequest).createQueryBuilder('r').where('r.id = :id AND r.userId = :userId', { id, userId }).setLock('pessimistic_write').getOne();
      ...
      const wallet = await walletRepo.createQueryBuilder('w').where('w.userId = :userId AND w.currency = :currency', { userId, currency: request.currency }).setLock('pessimistic_write').getOne();
      if (!wallet) throw new NotFoundException('کیف پول پیدا نشد.');
      const balanceBefore = normalizeDecimal(wallet.balance);
      await manager.query(`UPDATE wallets SET balance = balance + CAST($1 AS numeric), "updatedAt" = NOW() WHERE id = $2`, [request.amount, wallet.id]);
      const afterWallet = await walletRepo.findOne({ where: { id: wallet.id } });
      if (!afterWallet) throw new NotFoundException('کیف پول پس از بازگشت پیدا نشد.');
      const balanceAfter = normalizeDecimal(afterWallet.balance);
      request.status = 'CANCELLED'; request.cancelledAt = new Date(); await manager.getRepository(WithdrawalRequest).save(request);
      await manager.getRepository(WalletTransaction).save(manager.getRepository(WalletTransaction).create({ userId, walletId: wallet.id, type: WITHDRAW_REFUND, amount: request.amount, balanceBefore, balanceAfter, currency: request.currency, referenceType: REFERENCE, referenceId: request.id, description: 'بازگشت مبلغ درخواست برداشت لغوشده' }));
      return request;
    });
    try { await this.notifications.create(userId, { type: 'WITHDRAWAL_CANCELLED', title: 'درخواست برداشت لغو شد', message: `مبلغ ${result.amount} ${result.currency} به کیف پول شما بازگردانده شد.`, data: { withdrawalId: result.id, amount: result.amount, status: result.status } }); } catch {}
    return result;
  }
  ...
  async adminList(page = 1, limit = 10, status?: string) {
    ...
    return { items: items.map((item) => { const user = byId.get(item.userId); return { ...item, user: user ? { id: user.id, telegramId: user.telegramId, username: user.username, firstName: user.firstName, lastName: user.lastName, photoUrl: user.photoUrl } : null; } }), page, limit, total, pages: Math.ceil(total / limit) };
  }
  ...
  async adminComplete(id: string, adminUserId: string, receiptPath: string) {
    ...
    const result = await this.dataSource.transaction(async (manager) => {
      const request = await manager.getRepository(WithdrawalRequest).createQueryBuilder('r').where('r.id = :id', { id }).setLock('pessimistic_write').getOne();
      if (!request) throw new NotFoundException('درخواست برداشت پیدا نشد.');
    ...
    try { await this.notifications.create(result.userId, { type: 'WITHDRAWAL_COMPLETED', title: 'برداشت شما انجام شد', message: `درخواست برداشت ${result.amount} ${result.currency} واریز شد.`, data: { withdrawalId: result.id, amount: result.amount, status: result.status } }); } catch {}
    return result;
  }
}
