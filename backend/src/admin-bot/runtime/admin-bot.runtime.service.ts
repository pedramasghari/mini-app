...
export class AdminBotRuntimeService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit() {
    ...
    if (!this.adminIds().length) {
      this.logger.warn('ADMIN_TELEGRAM_IDS تنظیم نشده است؛ کاربران غیرادمین فقط Mini App را می‌بینند.');
    }
    ...
    try {
      ...
      this.running = false;
      this.logger.error(`Telegram bot initialization failed: ${error instanceof Error ? error.message : String(error)}`);
      throw error;
    }
  }
  ...
  async onModuleDestroy() {
    ...
    try {
      ...
      this.logger.warn(`Telegram bot stop failed: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  private adminIds() {
    return String(
      this.config.get('ADMIN_TELEGRAM_IDS') ?? this.config.get('ADMIN_TELEGRAM_ID') ?? '',
    )
      .split(',')
      .map((v) => v.trim())
      .filter(Boolean);
  ...
  private registerHandlers() {
    this.bot.command('start', async (ctx) => {
      this.conversations.clear(ctx.from.id);
      await this.showMain(ctx, ctx.from.id);
    });
    ...
    this.bot.callbackQuery(/^service:field:([^:]+):([^:]+)$/, async (ctx) => {
      await this.answerCallback(ctx);
      if (!this.isAdmin(Number(ctx.from.id))) return;
      await this.startServiceField(ctx, Number(ctx.from.id), ctx.match[1], ctx.match[2]);
    });
    ...
    this.bot.on('message:text', async (ctx) => {
      ...
      if (state) {
        try {
          ...
          this.logger.error(`Conversation error: ${error instanceof Error ? error.message : String(error)}`);
          await this.send(ctx, `❌ عملیات انجام نشد.\n${error instanceof Error ? error.message : 'خطای نامشخص'}`, AdminBotKeyboard.back());
        }
        return;
      }
  ...
  private async answerCallback(ctx: Context) {
    ...
    if (!this.isAdmin(id)) {
      await ctx.answerCallbackQuery({ text: 'دسترسی مجاز نیست.', show_alert: true }).catch(() => undefined);
      return;
    }
    await ctx.answerCallbackQuery().catch(() => undefined);
  }

  private async send(ctx: Context, text: string, replyMarkup?: BotKeyboard) {
    return ctx.reply(text, replyMarkup ? { reply_markup: replyMarkup } : undefined);
  }

  private async sendToChat(chatId: number, text: string, replyMarkup?: BotKeyboard) {
    return this.bot.api.sendMessage(chatId, text, replyMarkup ? { reply_markup: replyMarkup } : undefined);
  }

  private async showMain(ctx: Context, id: number) {
    if (!ctx.chat) return;
    if (!this.isAdmin(id)) {
      await this.send(ctx, '👋 برای استفاده از خدمات، مینی‌اپ را باز کنید.', AdminBotKeyboard.user(this.appUrl()));
      return;
    }
    await this.send(ctx, '🤖 پنل مدیریت\n\nاز منوی زیر عملیات موردنظر را انتخاب کنید.', AdminBotKeyboard.main(this.appUrl()));
  }

  private async showServices(ctx: Context) {
    await this.send(ctx, '🛍 مدیریت سرویس‌ها\n\nسرویس‌ها، محتوای آموزشی، قوانین و FAQ را مدیریت کنید.', AdminBotKeyboard.services());
  }

  private async showFinance(ctx: Context) {
    await this.send(ctx, '💰 مدیریت مالی\n\nدرخواست‌های شارژ و روش‌های پرداخت را مدیریت کنید.', {
      inline_keyboard: [
  ...
  private async startServiceCreate(ctx: Context, adminId: number) {
    ...
    await this.send(ctx, '➕ افزودن سرویس\n\nمرحله ۱ از ۸\nشناسه انگلیسی سرویس را ارسال کنید.\nمثال: apple-id', AdminBotKeyboard.cancel('admin:services'));
  }

  private async startServiceField(ctx: Context, adminId: number, serviceId: string, field: string) {
    ...
    await this.send(ctx, `✏️ ویرایش سرویس\n\n${prompts[field] ?? 'مقدار جدید را ارسال کنید.'}`, AdminBotKeyboard.cancel(`service:edit:${serviceId}`));
  }

  private async handleConversation(ctx: Context, adminId: number, text: string, state: any) {
    if (!text) {
    ...
    if (state.type === 'payment-reject') {
      ...
      await this.send(ctx, '❌ درخواست شارژ رد شد و دلیل برای کاربر ارسال شد.', {
        inline_keyboard: [
    ...
    if (state.type === 'service-edit') {
      ...
      this.conversations.clear(adminId);
      await this.send(ctx, `✅ سرویس «${service.title}» به‌روزرسانی شد.`, serviceMenu(service));
      return;
    }
    ...
    this.conversations.clear(adminId);
    await this.send(ctx, `✅ سرویس «${service.title}» ساخته شد.`, serviceMenu(service));
  }

  private async listServices(ctx: Context, mode?: 'edit' | 'delete') {
    const rows = await this.commerce.listServices(true);
    if (!rows.length) {
      await this.send(ctx, 'سرویسی ثبت نشده است.', AdminBotKeyboard.back('admin:services'));
      return;
    }
    ...
    const buttons = rows.map((s) => [
      {
        ...
        callback_data: mode === 'delete' ? `service:delete:confirm:${s.id}` : `service:edit:${s.id}`,
      },
    ]);
  ...
  private async toggleService(ctx: Context, id: string) {
    ...
    await this.send(ctx, `✅ سرویس «${updated.title}» ${updated.active ? 'فعال' : 'غیرفعال'} شد.`, serviceMenu(updated));
  }

  private async confirmDelete(ctx: Context, id: string) {
    const service = await this.commerce.getService(id);
    await this.send(ctx, `⚠️ سرویس «${service.title}» غیرفعال شود؟\nسفارش‌های قبلی حفظ خواهند شد.`, {
      inline_keyboard: [[
  ...
  private async deleteService(ctx: Context, id: string) {
    ...
    await this.send(ctx, `✅ سرویس «${service.title}» غیرفعال شد.`, AdminBotKeyboard.services());
  }
  ...
  private async listPendingPayments(ctx: Context) {
    ...
    if (!rows.length) {
      await this.send(ctx, '✅ درخواست شارژ در انتظاری وجود ندارد.', { inline_keyboard: [[{ text: '⬅️ مدیریت مالی', callback_data: 'admin:finance' }]] });
      return;
    }
    ...
    for (const payment of rows) {
      ...
      await this.send(ctx, text, {
        inline_keyboard: [
          [{ text: '📷 مشاهده فیش', callback_data: `payment:receipt:${payment.id}` }],
          [{ text: '✅ تأیید شارژ', callback_data: `payment:approve:${payment.id}` }, { text: '❌ رد درخواست', callback_data: `payment:reject:${payment.id}` }],
        ],
      });
    }
  }
  ...
  private async approvePayment(ctx: Context, id: string) {
    ...
    await this.send(ctx, '✅ درخواست شارژ تأیید شد و موجودی کاربر افزایش یافت.', {
      inline_keyboard: [
  ...
  private async startRejectPayment(ctx: Context, adminId: number, id: string) {
    ...
    await this.send(ctx, '✍️ دلیل رد درخواست را ارسال کنید:', AdminBotKeyboard.cancel('finance:pending'));
  }

  private async sendReceipt(ctx: Context, id: string) {
    const payment: any = await this.payments.findOne({ where: { id } });
    if (!payment) {
      await this.send(ctx, 'فیش پیدا نشد.', AdminBotKeyboard.back('finance:pending'));
      return;
    }
    ...
    try {
      ...
      this.logger.warn(`Receipt send failed: ${error instanceof Error ? error.message : String(error)}`);
  ...
  private async listPaymentMethods(ctx: Context) {
    ...
    if (!rows.length) {
      await this.send(ctx, 'روش پرداختی ثبت نشده است.', AdminBotKeyboard.back('admin:finance'));
      return;
    }
    ...
    await this.send(ctx, `💳 روش‌های پرداخت\n\n${text}`, AdminBotKeyboard.back('admin:finance'));
  }

  private async listOrders(ctx: Context, page: number) {
    const take = 10;
    const rows: any[] = await this.orders.find({ order: { createdAt: 'DESC' }, skip: (page - 1) * take, take });
    if (!rows.length) {
      await this.send(ctx, 'سفارشی ثبت نشده است.', AdminBotKeyboard.back('admin:orders'));
      return;
    }
    ...
    await this.send(ctx, `📦 سفارش‌های اخیر\n\n${text}`, {
      inline_keyboard: [[{ text: '⬅️ مدیریت سفارشات', callback_data: 'admin:orders' }]],
    });
  }

  private async listUsers(ctx: Context, page: number) {
    const take = 10;
    const rows: any[] = await this.users.find({ order: { createdAt: 'DESC' }, skip: (page - 1) * take, take });
    if (!rows.length) {
      await this.send(ctx, 'کاربری ثبت نشده است.', AdminBotKeyboard.back('admin:users'));
      return;
    }

    const text = rows
      .map((u) => `👤 ${[u.firstName, u.lastName].filter(Boolean).join(' ') || 'بدون نام'}\n🆔 ${u.telegramId ?? '-'}\n👤 @${u.username ?? '-'}`)
      .join('\n\n');

    await this.send(ctx, `👥 کاربران\n\n${text}`, {
      inline_keyboard: [[{ text: '⬅️ مدیریت کاربران', callback_data: 'admin:users' }]],
    });
  }
