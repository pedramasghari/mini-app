...
export class SupportChatService {
  ...
  async listForAdmins() {
    ...
    return conversations.map((conversation) => ({
      ...conversation,
      user: usersById.get(conversation.userId) ?? null,
    }));
  }
  ...
  async getForAdmin(conversationId: string) {
    ...
    await this.messages.update(
      { conversationId, senderRole: 'USER', readAt: IsNull() },
      { readAt: new Date(), status: 'READ' },
    );
    ...
    return {
      conversation,
      user: await this.users.findById(conversation.userId),
      messages: await this.messages.find({
  ...
  async send(
    ...
    if (role === 'USER' && conversation.userId !== userId)
      throw new NotFoundException('گفتگوی پشتیبانی پیدا نشد.');
    ...
    conversation.lastMessagePreview =
      body || (attachments[0]?.type === 'IMAGE' ? '📷 تصویر' : '🎬 ویدیو');
    ...
    if (!online) {
      ...
      if (user?.telegramId)
        await this.telegram
          .sendToTelegram(
            user.telegramId,
            `💬 ${title}\n${message}\n\nبرای مشاهده گفتگو وارد مینی‌اپ شوید.`,
          )
          .catch(() => undefined);
    }
  }
