...
export class AdminController {
  constructor(private readonly commerce: CommerceService, private readonly smsCode: SmsCodeService) {}
  ...
  @Get('smscode/operators') operators(@Req() req: Request) {
    ...
    const countryId = Number(query.countryId); const platformId = Number(query.platformId);
    if (!Number.isInteger(countryId) || !Number.isInteger(platformId)) throw new BadRequestException('countryId و platformId الزامی هستند.');
  ...
  @Get('smscode/products') catalogProducts(@Req() req: Request) {
    ...
    return this.smsCode.catalogProducts({ countryId, platformId, operatorId, sort: query.sort, page: query.page ? Number(query.page) : 1, limit: query.limit ? Number(query.limit) : 100 });
  }
  ...
  @UseInterceptors(FileInterceptor('file', {
    storage: diskStorage({ destination: GUIDE_UPLOAD_DIR, filename: (_request, file, callback) => callback(null, `${randomUUID()}${extname(file.originalname).toLowerCase()}`) }),
    limits: { fileSize: 30 * 1024 * 1024 },
    fileFilter: (_request, file, callback) => {
      if (!IMAGE_TYPES.has(file.mimetype) && !VIDEO_TYPES.has(file.mimetype)) { callback(new BadRequestException('فقط تصویر JPG/PNG/WebP/GIF یا ویدیوی MP4/WebM/MOV مجاز است.'), false); return; }
      callback(null, true);
    },
  }))
