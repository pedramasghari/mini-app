import {
  BadRequestException, Body, Controller, Delete, Get, Param, Patch, Post, Put, Query, Req, UploadedFile, UseGuards, UseInterceptors,
} from '@nestjs/common';
...
export class AdminController {
  ...
  async payments(@Query('page') pageQuery?: string, @Query('limit') limitQuery?: string, @Query('status') statusQuery?: string) {
    const page = Math.max(1, Number(pageQuery) || 1), limit = Math.min(50, Math.max(1, Number(limitQuery) || 10)), status = statusQuery?.trim().toUpperCase();
  ...
  async payment(@Param('id') id: string) {
    ...
    return { ...payment, user: user ? { id: user.id, telegramId: user.telegramId, username: user.username, firstName: user.firstName, lastName: user.lastName, photoUrl: user.photoUrl } : null };
  }

  @Post('payments/:id/approve') approvePayment(@Param('id') id: string, @Body('reason') reason?: string) { return this.commerce.approvePayment(id, reason); }
  ...
  uploadGuideMedia(@UploadedFile() file: Express.Multer.File) { if (!file) throw new BadRequestException('فایل ارسال نشده است.'); return { url: `/uploads/guides/${file.filename}`, mediaType: IMAGE_TYPES.has(file.mimetype) ? 'image' : 'video', originalName: file.originalname, size: file.size, mimeType: file.mimetype }; }
