...
export class AdminController {
  constructor(private readonly commerce: CommerceService, private readonly smsCode: SmsCodeService, @InjectRepository(PaymentRequest) private readonly paymentRequests: Repository<PaymentRequest>, @InjectRepository(User) private readonly users: Repository<User>) {}
  @Get('access') access(@Req() request: Request & { adminSession?: { user: unknown } }) { return { allowed: true, user: request.adminSession?.user ?? null }; }
  @Get('payments') async payments(@Query('page') pageQuery?: string, @Query('limit') limitQuery?: string, @Query('status') statusQuery?: string) { const page = Math.max(1, Number(pageQuery) || 1), limit = Math.min(50, Math.max(1, Number(limitQuery) || 10)), status = statusQuery?.trim().toUpperCase(); const qb = this.paymentRequests.createQueryBuilder('p').orderBy('p.createdAt', 'DESC'); if (status) qb.where('p.status = :status', { status }); const [items, total] = await qb.skip((page - 1) * limit).take(limit).getManyAndCount(); const userIds = [...new Set(items.map((item) => item.userId))], users = userIds.length ? await this.users.find({ where: userIds.map((id) => ({ id })) }) : [], byId = new Map(users.map((user) => [user.id, user])); return { items: items.map((item) => { const user = byId.get(item.userId); return { ...item, user: user ? { id: user.id, telegramId: user.telegramId, username: user.username, firstName: user.firstName, lastName: user.lastName, photoUrl: user.photoUrl } : null }; }), page, limit, total, pages: Math.ceil(total / limit) }; }
  @Get('payments/:id') async payment(@Param('id') id: string) { const payment = await this.paymentRequests.findOne({ where: { id } }); if (!payment) throw new BadRequestException('درخواست شارژ پیدا نشد.'); const user = await this.users.findOne({ where: { id: payment.userId } }); return { ...payment, user: user ? { id: user.id, telegramId: user.telegramId, username: user.username, firstName: user.firstName, lastName: user.lastName, photoUrl: user.photoUrl } : null }; }
  @Post('payments/:id/approve') approvePayment(@Param('id') id: string, @Body('amount') amount?: string, @Body('reason') reason?: string) { const value = String(amount ?? '').trim().replace(/,/g, ''); if (!/^(?:0|[1-9]\d*)(?:\.\d{1,8})?$/.test(value) || Number(value) <= 0) throw new BadRequestException('مبلغ نهایی شارژ را ادمین باید وارد کند.'); return this.commerce.approvePayment(id, value, reason); }
  @Post('payments/:id/reject') rejectPayment(@Param('id') id: string, @Body('reason') reason?: string) { const value = reason?.trim(); if (!value) throw new BadRequestException('دلیل رد درخواست الزامی است.'); if (value.length > 1000) throw new BadRequestException('دلیل رد درخواست بیش از حد طولانی است.'); return this.commerce.rejectPayment(id, value); }
  ...
  @Get('smscode/operators') operators(@Req() req: Request) { const query = req.query as Record<string, string | undefined>, countryId = Number(query.countryId), platformId = Number(query.platformId); if (!Number.isInteger(countryId) || !Number.isInteger(platformId)) throw new BadRequestException('countryId و platformId الزامی هستند.'); return this.smsCode.catalogOperators(countryId, platformId); }
  ...
  @Post('guide-media/upload') @UseInterceptors(FileInterceptor('file', { storage: diskStorage({ destination: GUIDE_UPLOAD_DIR, filename: (_request, file, callback) => callback(null, `${randomUUID()}${extname(file.originalname).toLowerCase()}`) }), limits: { fileSize: 30 * 1024 * 1024 }, fileFilter: (_request, file, callback) => { if (!IMAGE_TYPES.has(file.mimetype) && !VIDEO_TYPES.has(file.mimetype)) { callback(new BadRequestException('فقط تصویر JPG/PNG/WebP/GIF یا ویدیوی MP4/WebM/MOV مجاز است.'), false); return; } callback(null, true); } })) uploadGuideMedia(@UploadedFile() file: Express.Multer.File) { if (!file) throw new BadRequestException('فایل ارسال نشده است.'); return { url: `/uploads/guides/${file.filename}`, mediaType: IMAGE_TYPES.has(file.mimetype) ? 'image' : 'video', originalName: file.originalname, size: file.size, mimeType: file.mimetype }; }
