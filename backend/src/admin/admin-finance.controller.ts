...
export class AdminFinanceController {
  ...
  private async providerBalance() {
    ...
    return { balance: smscode.balance, currency: smscode.currency, available: true };
  }
  ...
  async listTransactions(
    ...
    if (userId?.trim()) qb.andWhere('t.userId = :userId', { userId: userId.trim() });
    const q = (search ?? telegramId ?? '').trim();
    if (q) qb.andWhere('(CAST(u."telegramId" AS TEXT) ILIKE :q OR COALESCE(u.username, \'\') ILIKE :q OR COALESCE(u.firstName, \'\') ILIKE :q OR COALESCE(u.lastName, \'\') ILIKE :q OR CAST(t.id AS TEXT) ILIKE :q OR COALESCE(t.referenceId, \'\') ILIKE :q OR COALESCE(t.description, \'\') ILIKE :q)', { q: `%${q}%` });
    if (status?.trim()) {
      switch (status.trim().toUpperCase()) {
        case 'SERVICE_PURCHASE':
          ...
          break;
        case 'SERVICE_REFUND': qb.andWhere("t.type = 'SMSCODE_ORDER_REFUND'"); break;
        case 'DEPOSIT': qb.andWhere("t.type = 'DEPOSIT'"); break;
        case 'WITHDRAW': qb.andWhere("LOWER(t.type) LIKE '%withdraw%'"); break;
        default: qb.andWhere('LOWER(t.type) = LOWER(:status)', { status: status.trim() });
      }
    }
