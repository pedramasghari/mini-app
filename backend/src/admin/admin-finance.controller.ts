...
export class AdminFinanceController {
  ...
  async overview() {
    ...
    return { smscode: await this.providerBalance(), usersBalance: wallet?.balance ?? '0', walletCount: Number(wallet?.walletCount ?? 0), totalReceived: received?.total ?? '0', totalWithdrawals: withdrawals?.total ?? '0', serviceRevenue: { standardOrders: serviceRevenue?.total ?? '0', smsCodeOrders: smsRevenue?.total ?? '0', total: (Number(serviceRevenue?.total ?? 0) + Number(smsRevenue?.total ?? 0)).toFixed(8) } };
  }
  ...
  async listTransactions(@Query('page') pageValue?: string, @Query('limit') limitValue?: string, @Query('telegramId') telegramId?: string, @Query('status') status?: string) {
    ...
    const qb = this.transactions.createQueryBuilder('t').leftJoin(User, 'u', 'u.id = t.userId').orderBy('t.createdAt', 'DESC');
    ...
    const userIds = [...new Set(items.map((item) => item.userId))]; const users = userIds.length ? await this.users.find({ where: userIds.map((id) => ({ id })) }) : []; const usersById = new Map(users.map((user) => [user.id, user]));
  ...
  async orderDetail(@Param('id') id: string, @Query('kind') kind?: string) {
    ...
    const order = await this.orders.findOne({ where: { id } }); if (!order) { const sms = await this.smsOrders.findOne({ where: { id } }); if (!sms) return { found: false }; return this.smsOrderDetail(id); }
    ...
    const service = product ? await this.services.findOne({ where: { id: product.serviceId } }) : null; return { found: true, kind: 'ORDER', order, product, service, user, inputs, progress };
  }
  private async smsOrderDetail(id: string) { const order = await this.smsOrders.findOne({ where: { id } }); if (!order) return { found: false }; const [user, product, service] = await Promise.all([this.users.findOne({ where: { id: order.userId } }), order.productId ? this.products.findOne({ where: { id: order.productId } }) : null, order.serviceId ? this.services.findOne({ where: { id: order.serviceId } }) : null]); return { found: true, kind: 'SMSCODE', order, product, service, user }; }
}
