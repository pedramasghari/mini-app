...
export class CommerceService implements OnModuleInit {
  constructor(
    ...
    @InjectRepository(ActivationGuide)
    private guides: Repository<ActivationGuide>,
    @InjectRepository(ActivationStep) private steps: Repository<ActivationStep>,
    @InjectRepository(ActivationProgress)
    private progress: Repository<ActivationProgress>,
    ...
    private payments: Repository<PaymentRequest>,
    @InjectRepository(WalletTransaction)
    private transactions: Repository<WalletTransaction>,
  ...
  async onModuleInit() {
    ...
    const service = await this.services.save(
      this.services.create({
        slug: 'apple-id',
        title: 'Apple ID',
        description: 'Guided Apple account setup service.',
        icon: 'apple',
        active: true,
      }),
    );
    const product = await this.products.save(
      this.products.create({
        ...
        title: 'Apple ID Setup',
        description:
          'Step-by-step guidance for setting up your own Apple Account.',
        price: '9.99',
        currency: 'USD',
        icon: 'apple',
        active: true,
    ...
    const guide = await this.guides.save(
      this.guides.create({
        ...
        title: 'Apple Account Setup Guide',
        description:
          'Follow each step in order and use your own account information.',
        active: true,
      }),
    );
    await this.steps.save([
      this.steps.create({
        ...
        content:
          'Have an email address you control and the information required by Apple ready before continuing.',
      }),
      this.steps.create({
        guideId: guide.id,
        position: 2,
        title: 'Start Apple Account setup',
        content:
          'Open Apple’s official account setup flow and follow the on-screen instructions.',
      }),
      this.steps.create({
        guideId: guide.id,
        position: 3,
        ...
        content:
          'Enter your own name, date of birth, email address and a strong password when requested.',
        requiresInput: true,
        inputKey: 'email',
      ...
      this.steps.create({
        ...
        content:
          'Complete any verification Apple requests using verification methods available to you.',
      }),
      this.steps.create({
        guideId: guide.id,
        position: 5,
        title: 'Finish setup',
        content:
          'Review the account details, accept Apple’s terms where applicable, and finish the setup.',
      }),
    ]);
  }
  ...
  async product(id: string) {
    const p = await this.products.findOne({ where: { id, active: true } });
    ...
    const result = await this.dataSource.transaction(async (manager) => {
      ...
      if (wallet.currency !== product.currency)
        throw new BadRequestException(
          `Wallet currency must be ${product.currency}`,
        );
      ...
      const guide = await manager.findOne(ActivationGuide, {
        where: { productId, active: true },
      });
      let progress: ActivationProgress | null = null;
      if (guide)
        progress = await manager.save(
          manager.create(ActivationProgress, {
            orderId: saved.id,
            userId,
            guideId: guide.id,
            currentStep: 0,
  ...
  async orderGuide(userId: string, orderId: string) {
    ...
    if (!progress)
      progress = await this.progress.save(
  ...
  async updateProgress(
    ...
    progress.completed = step === data.steps.length - 1;
  ...
  async saveInputs(
    ...
    const rows = Object.entries(values)
      ...
      .map(([key, value]) => this.inputs.create({ orderId, key, value }));
  ...
  async paymentMethods() {
    return this.methods.find({
      where: { active: true },
      select: ['id', 'type', 'title', 'cardNumber', 'holderName', 'bankName'],
    });
  }
  async createPayment(
    userId: string,
    ...
    return this.payments.save(
      this.payments.create({ userId, paymentMethodId, amount, receiptPath }),
    );
  }
  async approvePayment(paymentId: string, reason?: string) {
    return this.dataSource.transaction(async (manager) => {
      const payment = await manager.findOne(PaymentRequest, {
        where: { id: paymentId },
      });
  ...
  async rejectPayment(paymentId: string, reason: string) {
    ...
    payment.adminReason = reason || 'Rejected by admin';
