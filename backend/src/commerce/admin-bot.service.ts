...
export class AdminBotService implements OnModuleInit, OnModuleDestroy {
  ...
  private async handle(update: any) {
    ...
    if (!message?.from || !this.allowed(message.from.id)) return;
    const text = String(message.text ?? '').trim();
    const state = this.states.get(message.from.id);
  ...
  private async stateMessage(chatId: number, adminId: number, text: string, state: { action: string; values: string[]; paymentId?: string }) {
    if (state.action === 'reject') {
      ...
      await this.commerce.rejectPayment(state.paymentId!, text);
      return this.send(chatId, 'Payment rejected.');
    }
    if (state.action === 'addcard') {
      state.values.push(text);
      if (state.values.length === 1) return this.send(chatId, 'Card holder name:');
      if (state.values.length === 2) return this.send(chatId, 'Bank name (or -):');
      this.states.delete(adminId);
      const [cardNumber, holderName, bank] = state.values;
      await this.methods.save(this.methods.create({ type: 'CARD_TRANSFER', title: `${bank === '-' ? '' : bank + ' '}Card`, cardNumber, holderName, bankName: bank === '-' ? null : bank, active: true }));
      return this.send(chatId, `Card added.\n${cardNumber.slice(0, 4)} **** **** ${cardNumber.slice(-4)}`);
    }
  }

  private async callback(query: any) {
    const adminId = query.from?.id;
    if (!adminId || !this.allowed(adminId)) return;
    const data = String(query.data ?? '');
    await this.api('answerCallbackQuery', { callback_query_id: query.id });
    if (data.startsWith('approve:')) {
      const id = data.slice(8); await this.commerce.approvePayment(id, 'Approved by admin'); return this.send(query.message.chat.id, `Payment ${id} approved.`);
    }
    if (data.startsWith('reject:')) {
      const id = data.slice(7); this.states.set(adminId, { action: 'reject', values: [], paymentId: id }); return this.send(query.message.chat.id, 'Send rejection reason:');
    }
  }

  private async sendPending(chatId: number) {
    const rows = await this.commerce.pendingPayments();
    if (!rows.length) return this.send(chatId, 'No pending payments.');
    for (const p of rows) {
      const user = await this.users.findOne({ where: { id: p.userId } });
      await this.send(chatId, `Payment\nID: ${p.id}\nUser: ${user?.username ? '@' + user.username : user?.telegramId}\nAmount: ${p.amount} ${p.currency}\nStatus: ${p.status}`, {
        inline_keyboard: [[{ text: '✅ Approve', callback_data: `approve:${p.id}` }, { text: '❌ Reject', callback_data: `reject:${p.id}` }]],
      });
  ...
  private async sendCards(chatId: number) {
    ...
    if (!cards.length) return this.send(chatId, 'No active cards.');
    return this.send(chatId, cards.map(c => `• ${c.bankName ?? ''} ${c.cardNumber.slice(0, 4)} **** **** ${c.cardNumber.slice(-4)} — ${c.holderName}`).join('\n'));
  }

  private async send(chatId: number, text: string, inline_keyboard?: any[][]) {
    return this.api('sendMessage', { chat_id: chatId, text, ...(inline_keyboard ? { reply_markup: { inline_keyboard } } : {}) });
  }

  private async sendFile(chatId: number, path: string, caption: string) {
    const token = this.config.get<string>('ADMIN_BOT_TOKEN');
    const bytes = await readFile(path);
    const form = new FormData();
    form.append('chat_id', String(chatId));
    form.append('caption', caption);
    form.append('document', new Blob([bytes]), path.split(/[\\/]/).pop() ?? 'receipt');
