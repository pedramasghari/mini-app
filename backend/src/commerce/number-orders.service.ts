...
import {
  ...
  NumberOrderOtp,
  NumberOrderStatus,
} from './entities/number-order.entity';
...
export class NumberOrdersService implements OnModuleInit, OnModuleDestroy {
  ...
  async createOrUpdate(
    ...
    if (!product || !row.serviceId || !row.productId)
      throw new NotFoundException('محصول سفارش شماره پیدا نشد.');

    let order = await this.numberOrders.findOne({
      where: { smsCodeOrderId: row.id },
    });
  ...
  private async syncRow(row: SmsCodeOrder) {
    try {
      const snapshot = (await this.smsCode.get(
        row.userId,
        row.id,
      )) as SmsSnapshot;
    ...
    for (const order of rows) {
      ...
      const transactions = await this.transactions.find({
        where: {
          ...
          referenceId: order.smsCodeOrderId,
        },
        order: { createdAt: 'DESC' },
      });
  ...
  async listActive(userId: string): Promise<ActiveNumberOrder[]> {
    ...
    for (const order of rows) {
      const smsRow = await this.smsOrders.findOne({
        where: {
