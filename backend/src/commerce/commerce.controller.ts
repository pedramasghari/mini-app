...
export class CommerceController {
  ...
  @Post('orders') async createOrder(@Req() req: Request, @Body('productId') productId: string) {
    if (!productId) throw new BadRequestException('productId is required');
    return this.commerce.createOrder(await this.userId(req), productId);
  }

  @Post('orders/:id/inputs') async inputs(@Req() req: Request, @Param('id') id: string, @Body() values: Record<string, string>) {
    return this.commerce.saveInputs(await this.userId(req), id, values);
  }
  ...
  async cardTransfer(@Req() req: Request, @Body('amount') amount: string, @Body('paymentMethodId') methodId: string, @UploadedFile() receipt?: Express.Multer.File) {
    if (!receipt) throw new BadRequestException('Receipt is required');
    return this.commerce.createPayment(await this.userId(req), amount, methodId, receipt.path);
  }
}
